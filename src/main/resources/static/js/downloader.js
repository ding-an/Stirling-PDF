...
function showCostIframe() {
	...
	div.style.display = 'flex' // Hide the banner by default
	div.style.alignItems = 'center' // Center align the content inside the banner
	div.style.justifyContent = 'center' // Center align the content inside the banner

	var iframe = document.createElement('iframe')

	// 设置iframe的属性
	iframe.src = 'https://xbwl-fe-payment-site.vercel.app/consume'
	iframe.width = '400px'
	iframe.height = '190px'
	...
	$('form').submit(async function (event) {
		...
		try {
			if (remoteCall === true) {
				if (
					override === 'multi' ||
					(!multipleInputsForSingleRequest && files.length > 1 && override !== 'single')
				) {
